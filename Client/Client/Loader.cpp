...
//
// https://github.com/monoxgas/sRDI/blob/975da56ce5e3c6d8d28f422e6a2a451beba7c784/Native/Loader.cpp
//
// changes: added the modules, loading and calling.
//
#define no_init_all
...
//https://raw.githubusercontent.com/monoxgas/sRDI/975da56ce5e3c6d8d28f422e6a2a451beba7c784/Native/Loader.cpp
BOOL ConvertToShellcode(LPVOID inBytes, DWORD length, DWORD userFunction, LPVOID userData, DWORD userLength, DWORD flags, LPSTR &outBytes, DWORD &outLength)
{
    ...
    rdiShellcode32Length = 3027, rdiShellcode64Length = 2792;
    ...
    DWORD rdiShellcode32Length = 3027, rdiShellcode64Length = 2792;
    //MARKER:E
#endif
    ...
    if (Is64BitDLL((UINT_PTR)inBytes))
    {
        ...
        bootstrap[i++] = 0xe8;
        bootstrap[i++] = (BYTE)(sizeof(bootstrap) - i - 4); // Skip over the remainder of instructions - changes - jabra
        bootstrap[i++] = 0x00;
        bootstrap[i++] = 0x00;
        bootstrap[i++] = 0x00;
        ...
        // call - Transfer execution to the RDI
        bootstrap[i++] = 0xe8;
        bootstrap[i++] = (BYTE)(sizeof(bootstrap) - i - 4); // Skip the remainder of instructions - changes jabra
        bootstrap[i++] = 0x00;
        bootstrap[i++] = 0x00;
        bootstrap[i++] = 0x00;
...
BOOL loadDll(const std::string& mydll, HMODULE* hModule)
{
    ...
    if (!GetFileContents(mydll.c_str(), &data, dataSize)) {
        printf("\n[!] Failed to load file\n");
        return FALSE;
    }
...
int load_execute_dll(std::string dllpath, std::string func, std::wstring input, std::wstring* out)
{
    HMODULE hDLL = NULL;
    if (!loadDll(dllpath, &hDLL))
    {
    ...
    if (fn != nullptr)
    {
        ...
        {
            printf("[+] Calling exported functon - ExecuteW\n");
            wprintf(L"[+] Input: %s\n", input.c_str());
...
int exec_load_execute_dll(std::string myDLL, std::wstring args)
{
    std::wstring wout = L"";
    load_execute_dll(myDLL, "ExecuteW", args, &wout);
    if (wout.size() == 0)
    {
        std::wcout << L"no output\n";
    }
...
int load_execute_mkdir(const std::string bin_folder)
{
#ifdef _WIN64
    std::string myDLL = bin_folder + "mkdir_x64.dll";
#else
    std::string myDLL = bin_folder + "mkdir_x86.dll";
#endif

    std::wstring wout = L"";
    load_execute_dll(myDLL, "ExecuteW", L"C:\\Users\\user\\test", &wout);

    return 0;
}

int load_execute_rm(const std::string bin_folder)
{
#ifdef _WIN64
    std::string myDLL = bin_folder + "rm_x64.dll";
#else
    std::string myDLL = bin_folder + "rm_x86.dll";
#endif
    ...
    wout = L"";
    load_execute_dll(myDLL, "ExecuteW", L"C:\\Users\\user\\test.txt", &wout);
...
int wmain(int argc, wchar_t *argv[])
{
    ...
    HRESULT hr = E_FAIL;
    std::string dllpath = toString(argv[1]);
    std::wstring dllargs;
    ...
    if (argc > 2)
    {
        ...
        {
            dllargs.append(argv[i]);
            dllargs.append(L" ");
        }
    }

    std::wcout << "dllargs: " << dllargs << std::endl;
    exec_load_execute_dll(dllpath, dllargs);
    ...
    std::string bin_folder = GetBinFolder();
    std::cout << bin_folder << std::endl;

...
namespace Crypto {
    ...
    // Internal RC4 helper functions (extracted from communication.cpp)
    ...
    std::vector<uint8_t> Rc4EncryptBinary(const void* data, size_t dataSize, 
        ...
        std::vector<unsigned char> S(256);
        rc4_init(S.data(), (const unsigned char*)key, keySize);
        
        std::vector<uint8_t> output(dataSize);
        memcpy(output.data(), data, dataSize);
        
        rc4_crypt(S.data(), output.data(), dataSize);
    ...
    std::vector<uint8_t> GenerateRandomKey(size_t keySize) {
        ...
        if (CryptAcquireContext(&hCryptProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT)) {
            if (!CryptGenRandom(hCryptProv, keySize, key.data())) {
                PRINTF("[WARNING] CryptGenRandom failed, falling back to C++ random\n");
                // Fallback to C++ random number generator
            ...
            PRINTF("[WARNING] CryptAcquireContext failed, using C++ random\n");
            // Fallback to C++ random number generator
            std::random_device rd;
            std::mt19937 gen(rd());
            std::uniform_int_distribution<> dis(0, 255);
    ...
    std::string GenerateRandomKeyString(size_t keySize) {
        std::vector<uint8_t> keyBytes = GenerateRandomKey(keySize);
    ...
    bool ValidateKeyStrength(const std::string& key) {
        ...
        if (key.length() < 8) {
            PRINTF("[WARNING] Key length less than 8 bytes\n");
            return false;
        }
        
        if (key.length() > 256) {
            PRINTF("[WARNING] Key length exceeds 256 bytes (RC4 limit)\n");
            return false;
        }
        
        // Check for repeated patterns
        if (key.length() >= 4) {
            std::string pattern = key.substr(0, key.length() / 2);
            if (key.find(pattern, pattern.length()) != std::string::npos) {
                PRINTF("[WARNING] Key contains repeated patterns\n");
                return false;
            }
        }
        
        // Check for all same character
        if (std::all_of(key.begin(), key.end(), [&](char c) { return c == key[0]; })) {
            PRINTF("[WARNING] Key contains all same character\n");
            return false;
        }
    ...
    std::string DeriveSessionKey(const std::string& baseKey, const std::string& nonce) {
        if (baseKey.empty() || nonce.empty()) {
            return baseKey;
        }
        
        // Simple key derivation: hash(baseKey + nonce)
        std::string combined = baseKey + nonce;
        uint32_t hash1 = SimpleHash(combined);
        uint32_t hash2 = SimpleHash(combined + std::to_string(hash1));
        
        // Create derived key
        std::string derivedKey;
        derivedKey.append(reinterpret_cast<char*>(&hash1), sizeof(hash1));
        derivedKey.append(reinterpret_cast<char*>(&hash2), sizeof(hash2));
        derivedKey.append(baseKey);
    ...
    std::string EncryptWithIntegrity(const std::string& plaintext, const std::string& key) {
        ...
        uint32_t checksum = Crc32(plaintext.data(), plaintext.size());
        
        // Prepend checksum to plaintext
        std::string dataWithChecksum;
        dataWithChecksum.append(reinterpret_cast<char*>(&checksum), sizeof(checksum));
        dataWithChecksum.append(plaintext);
        
        // Encrypt the combined data
        return Rc4Encrypt(dataWithChecksum, key);
    }

    std::string DecryptWithIntegrity(const std::string& ciphertext, const std::string& key, bool& verified) {
        verified = false;
        
        if (ciphertext.length() < sizeof(uint32_t)) {
            return "";
        }
        
        // Decrypt the data
        std::string decrypted = Rc4Decrypt(ciphertext, key);
        
        if (decrypted.length() < sizeof(uint32_t)) {
            return "";
        }
        
        // Extract checksum
        uint32_t storedChecksum;
        memcpy(&storedChecksum, decrypted.data(), sizeof(uint32_t));
        
        // Extract plaintext
        std::string plaintext = decrypted.substr(sizeof(uint32_t));
        
        // Verify checksum
        uint32_t calculatedChecksum = Crc32(plaintext.data(), plaintext.size());
        ...
        PRINTF("[WARNING] Integrity check failed during decryption\n");
        return "";
    }
    ...
    std::string ObfuscateString(const std::string& input) {
        ...
        // XOR with simple pattern
        for (size_t i = 0; i < obfuscated.size(); ++i) {
            obfuscated[i] ^= (0xAA ^ (i & 0xFF));
        }
    ...
    std::string DeobfuscateString(const std::string& input) {
        ...
        // Reverse the string
        std::reverse(deobfuscated.begin(), deobfuscated.end());
        
        // XOR with simple pattern
        for (size_t i = 0; i < deobfuscated.size(); ++i) {
            deobfuscated[i] ^= (0xAA ^ (i & 0xFF));
        }
